...
echo "⚔️ TESTING WISDOM SHORTS LOCALLY WITH DYNAMIC STACKED CHUNKS + PERMANENT HOOK + CONSISTENT BOXCOLOR"

# 1️⃣ pick mood (can expand if you wish)
MOODS=("Resolute")
MOOD="${MOODS[$((RANDOM % ${#MOODS[@]}))]}"
echo "🟢 Mood chosen: $MOOD"

# 2️⃣ pick random Drive file ID from urls.txt
...
# 8️⃣ check if video long enough
...
if (( $(echo "$VIDEO_DURATION < $FINAL_DURATION" | bc -l) )); then
  ...
  SECOND_ID=$(shuf -n1 assets/$MOOD/background/urls.txt)
  gdown --fuzzy "https://drive.google.com/uc?id=$SECOND_ID" -O second_video.mp4
...
# 9️⃣ drawtext: permanent hook + stacking chunks
DRAW_TEXTS="drawtext=textfile='$INTRO':fontfile=$FONT:fontsize=64:fontcolor=$COLOR:box=1:boxcolor=$BOXCOLOR:boxborderw=12:x=(w-text_w)/2:y=40:enable='between(t,0,$FINAL_DURATION)',"

for ((i=0; i<CHUNK_COUNT; i++)); do
  y_offset=$(( 150 + i * 60 ))  # stack downward
  appear_time=${STARTS[i]}
  DRAW_TEXTS+="drawtext=text='${CHUNKS[i]}':fontfile=$FONT:fontsize=42:fontcolor=$COLOR:x=(w-text_w)/2:y=$y_offset:enable='gte(t,$appear_time)',"
done

DRAW_TEXTS+="drawtext=text='$OUTRO':fontfile=$FONT:fontsize=42:fontcolor=$COLOR:box=1:boxcolor=$BOXCOLOR:boxborderw=10:x=(w-text_w)/2:y=(h-text_h)-100:enable='between(t,${FINAL_DURATION}-3,${FINAL_DURATION})',"
...
echo "✅ Final wisdom video generated with permanent hook, stacked chunks, consistent boxcolor — empire ready."
